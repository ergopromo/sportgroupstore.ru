...
/*
 * Назначение: Шаблон отображения списка заказов пользователя в личном кабинете.
 * Выводит информацию о заказах, их оплате, доставке и предоставляет возможность оплаты заказов.
 * Кнопка "Оплатить" отображается только для заказов со статусом "Собран" (CO) и выше.
 *
 * История изменений:
 * v1.1 – Добавлена логика скрытия кнопки "Оплатить" до перехода заказа в статус "Собран" (CO) (2025-10-29)
...
// Принудительная сортировка заказов по ID в обратном порядке (новые первыми)
if (!empty($arResult['ORDERS'])) {
    usort($arResult['ORDERS'], function($a, $b) {
        return $b['ORDER']['ID'] - $a['ORDER']['ID'];
    });
}
...
else
{
	...
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		...
		{
			// Заголовок по статусам отключен - все заказы в едином списке
			
			// SI: Кнопка оплаты доступна только для заказов со статусом "Собран" (CO) и выше
			$allowPayButton = in_array($order['ORDER']['STATUS_ID'], array('CO', 'F'));
				...
				<div class="row">
					<div class="col-md-12 col-sm-12 col-xs-12 sale-order-list-title-container">
						<h2 class="sale-order-list-title">
							<?
                            $resultTitle = Loc::getMessage('SPOL_TPL_ORDER');
                            $resultTitle .= ' ' . Loc::getMessage('SPOL_TPL_NUMBER_SIGN').$order['ORDER']['ACCOUNT_NUMBER'];
                            $resultTitle .= ' ' . Loc::getMessage('SPOL_TPL_FROM_DATE');
                            $resultTitle .= ' ' . $order['ORDER']['DATE_INSERT']->format($arParams['ACTIVE_DATE_FORMAT']) . ',';

                            if(!empty($order['BASKET_ITEMS'])) {
                                $resultTitle .= ' ' . count($order['BASKET_ITEMS']);

                                $count = count($order['BASKET_ITEMS']) % 10;
                                if ($count == '1')
                                {
                                    $resultTitle .= ' ' .  Loc::getMessage('SPOL_TPL_GOOD');
                                }
                                elseif ($count >= '2' && $count <= '4')
                                {
                                    $resultTitle .= ' ' .  Loc::getMessage('SPOL_TPL_TWO_GOODS');
                                }
                                else
                                {
                                    $resultTitle .= ' ' .  Loc::getMessage('SPOL_TPL_GOODS');
                                }
                            }
                            $resultTitle .= ' ' .  Loc::getMessage('SPOL_TPL_SUMOF');
                            $resultTitle .= ' ' .  $order['ORDER']['FORMATED_PRICE'];
                            echo $resultTitle;
							?>
						</h2>
					</div>
						...
						foreach ($order['PAYMENT'] as $payment)
						{
							...
							<div class="row sale-order-list-inner-row">
								...
								<div class="col-12 sale-order-list-inner-row-body">
									...
									<?
					// Проверка возможности отображения кнопки оплаты
					if ($payment['PAID'] === 'N' && $payment['IS_CASH'] !== 'Y')
					{
						// Кнопка отображается только если разрешена оплата И статус заказа "Собран" или выше
						if ($allowPayButton && $order['ORDER']['IS_ALLOW_PAY'] !== 'N')
						{
	...
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		$javascriptParams = array(
			...
			"templateName" => 'sotbit_order_detail',
