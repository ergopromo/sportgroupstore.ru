...
$arParams['IMAGE_FOR_OFFER'] = Config::get('IMAGE_FOR_OFFER');
...
$ar_res = $res->GetNext();
$catalog = CIBlock::GetList(array(), array('CODE' => 'sotbit_origami_catalog_' . SITE_ID), true);

$arResult['CATALOG_PATH'] = SITE_DIR . str_replace('#SITE_DIR#/', '', $catalog->Fetch()['LIST_PAGE_URL']); //$ar_res['LIST_PAGE_URL'];
$arResult['BASKET_PATH'] = Config::get('BASKET_PAGE');
if(Config::get("HEADER_CALL") == "Y" && (Config::get('SHOW_BUY_OC') == "Y") && \Bitrix\Main\Loader::includeModule('sotbit.orderphone')) {
...
foreach ($arResult['CATEGORIES'] as &$parentCategory) {
    foreach ($parentCategory as &$item) {
        $priceRatio = (float)$item['RATIO'] * $item['PRICE'];
        $item['PRICE_FMT'] = \CCurrencyLang::CurrencyFormat($priceRatio, $arResult['CURRENCY'], true);
    }
}

$arResult["OLD_PRICE_ITOGO_TOTAL"] = $arResult['TOTAL_PRICE'];
