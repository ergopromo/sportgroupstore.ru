	...
	<div id="<?=$this->GetEditAreaID($item['ID'])?>" class="check_basket_<?=$item['ID']?> product_card__block_item_inner-wrapper">
		<div class="product_card__block_item_inner" id="<?=$areaId?>" data-entity="item">
            ...
            if (!$haveOffers)
            {
                ...
                $minOffers=$item['OFFERS'][0]['ITEM_PRICES'][0]['RATIO_PRICE'];
                $offersSelected = 0;
                foreach ($item['OFFERS'] as $key => $offers)
                {
                    if ($offers['ITEM_PRICES'][0]['RATIO_PRICE']<$minOffers)
                    {
                        $minOffers=$offers['ITEM_PRICES'][0]['RATIO_PRICE'];
                        $offersSelected=$key;
                    }
                }
                ...
                if (!empty($item['OFFERS_PROP']))
                {
                    ...
                    $jsParams['OFFER_SELECTED'] = $offersSelected;
            ...
            $messages = array(
                ...
                'RELATIVE_QUANTITY_NO_TEXT' => $arParams['MESS_RELATIVE_QUANTITY_NO'],
                'RELATIVE_QUANTITY_MANY_TEXT' => $arParams['MESS_RELATIVE_QUANTITY_MANY'],
                'RELATIVE_QUANTITY_FEW_TEXT' => $arParams['MESS_RELATIVE_QUANTITY_FEW'],
            );
