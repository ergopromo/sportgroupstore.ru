...
$selectSectionMain = unserialize(\Sotbit\Origami\Config\Option::get('SECTIONS_', SITE_ID));
$selectSectionNoTabs = unserialize(\Sotbit\Origami\Config\Option::get('SECTIONS_NO_TABS', SITE_ID));
$catalogId = Option::get('IBLOCK_ID', SITE_ID);
$ar = array();
...
if (in_array($sectionIdByCode, $arSectionsMain)) {
    ...
    if (\Sotbit\Origami\Config\Option::get('DETAIL_TEMPLATE', SITE_ID) == "") {
        $templateName = "";
        $template = "";
    } else if (\Sotbit\Origami\Config\Option::get('DETAIL_TEMPLATE', SITE_ID) == "NO_TABS") {
        $templateName = "origami_no_tabs";
        $template = "NO_TABS";
    }
}
$cacheParent = $componentElementParams['CACHE_TYPE'];
$componentElementParams['CACHE_TYPE'] = 'N';
...
$componentElementParams['CACHE_TYPE'] = $cacheParent;
...
if ($elementId > 0) {
    ...
    $showCrosssell = (\Sotbit\Origami\Config\Option::get('SHOW_CROSSSELL_' . $template, SITE_ID));
    if (!in_array($showCrosssell, array('Y', 'N'))) {
        $showCrosssell = 'N';
    }

    if ($showCrosssell == 'Y' && Loader::includeModule("sotbit.crosssell")) {
