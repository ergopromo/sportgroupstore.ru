...
/*
Назначение:
Основной файл шаблона компонента оформления заказа (sale.order.ajax).
Отвечает за общую структуру страницы, кастомную JS-логику
обработки AJAX-ответов и подключение составных частей шаблона
(свойства, доставка, оплата, итоги).

История
изменений:
v1.2
от 2025-10-22:
– В функции ajaxResult добавлен явный вызов BX.saleOrderAjax.initDeferredControl()
  после BX.ajax.processScripts для дополнительной инициализации
  элементов управления служб доставки (виджетов карт).
– Добавлена попытка повторной инициализации карт (Yandex/Google), если
  соответствующие объекты JS существуют.
v1.1
от 2025-10-22:
– Исправлена ошибка, при которой не отображался виджет выбора
  ПВЗ СДЭК (и других служб доставки) после AJAX-обновления.
– В функции ajaxResult заменена прямая вставка .innerHTML
  на BX.processHTML и BX.ajax.processScripts(..., true).
  Второй параметр 'true' обеспечивает выполнение скриптов,
  обернутых в BX.ready().
v1.0
от 2025-10-22:
– Исходная версия.
*/
                ...
                else
                {
                    ...
                    <script>
                        ...
                        function ajaxResult(res)
                        {
                            ...
                            catch(e)
                            {
                                ...
                                // SI: Использование BX.processHTML и BX.ajax.processScripts
                                // для корректной обработки скриптов, возвращаемых в AJAX-ответе.
                                // Второй параметр 'true' в processScripts обеспечивает
                                // выполнение обработчиков, завёрнутых в BX.ready
                                // (критично для виджетов служб доставки).
                                var processed = BX.processHTML(res);
                                BX('order_form_content').innerHTML = processed.HTML;
                                BX.ajax.processScripts(processed.SCRIPT, true);

                                // SI: Повторная инициализация отложенных контролов и карт
                                // после вставки HTML и выполнения скриптов.
                                if (BX.saleOrderAjax) {
                                    BX.saleOrderAjax.initDeferredControl();
                                }
                                // Попытка инициализации карт, если объекты существуют
                                if (typeof ymaps !== 'undefined' && typeof BX.Sale.OrderAjaxComponent !== 'undefined' && typeof BX.Sale.OrderAjaxComponent.initMaps === 'function') {
                                     ymaps.ready(BX.proxy(BX.Sale.OrderAjaxComponent.initMaps, BX.Sale.OrderAjaxComponent));
                                } else if (typeof google !== 'undefined' && typeof BX.Sale.OrderAjaxComponent !== 'undefined' && typeof BX.Sale.OrderAjaxComponent.initMaps === 'function') {
                                ...
                                <?if(CSaleLocation::isLocationProEnabled()):?>
                                    // SI: Эта строка уже была, но оставляем на случай,
                                    // если стандартный initDeferredControl не сработал выше.
                                    BX.saleOrderAjax.initDeferredControl();
                                <?endif?>
                            }
                    ...
                    if(!empty($arResult["ERROR"]) && $arResult["USER_VALS"]["FINAL_STEP"] == "Y")
                    {
                        ...
                        <div class="errors_list">
                            <?
                            foreach($arResult["ERROR"] as $v)
                                echo '<i class="fa fa-circle errortext">' . $v . '</i>';
                            ?>
                        </div>
                        <script>
                    ...
                    <div class="price_order_block">
                        ...
                        if(strlen($arResult["PAYED_FROM_ACCOUNT_FORMATED"]) > 0)
                        {
                            ...
                            <div class="price_order_block__item fonts__small_text">
                                ...
                                <b><?=$arResult["ORDER_TOTAL_LEFT_TO_PAY_FORMATED"]?></b>
                            </div>
                            <?
                        }
                        else
...
<script>
    ...
    function validation() {
        ...
        var form = $("#order_form_div");

        elems.each(function(i, elem) {
            if($(elem).hasClass("required") && !elem.value) {
                $(elem).addClass("has-error");
                form.addClass('has-errors');
            }else if($(elem).hasClass('has-error')){
                form.addClass("has-errors");
            }else {
