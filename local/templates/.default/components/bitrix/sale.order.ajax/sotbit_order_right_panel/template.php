...
$checkMsgs = function ($arMsgKeys, $arParams, $useDefaultMessages) {
    foreach ($arMsgKeys as $msgKey) {
        if($useDefaultMessages || !isset($arParams['MESS_' . $msgKey])) {
            $arParams['MESS_' . $msgKey] = Loc::getMessage($msgKey . '_DEFAULT');
        }
    }
...
$arParams = $checkMsgs($arMsgKeys, $arParams, $useDefaultMessages);
...
$arMsgKeys = [
    'PRELOAD_ORDER_TITLE',
    'SUCCESS_PRELOAD_TEXT',
    'FAIL_PRELOAD_TEXT',
    'DELIVERY_CALC_ERROR_TITLE',
    'DELIVERY_CALC_ERROR_TEXT',
    'PAY_SYSTEM_PAYABLE_ERROR'
];
...
else
{
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
            ...
            <div class="bx-sidebar col-md-12 col-12 row">
			<!--	SIDEBAR BLOCK	-->
