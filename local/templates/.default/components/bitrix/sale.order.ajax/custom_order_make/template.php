<?
/*
· Назначение: Кастомный шаблон оформления заказа (копия sotbit_order_make_classic). Функциональность сохранена без изменений; добавлены служебные комментарии и БЭМ-обёртка.
·
· История изменений:
· v1.2 – удалён дублирующийся <form> и скрытое поле PERMANENT_MODE_STEPS; подключён force_expand.js только один раз в конце (2025-10-22)
· v1.1 – добавлена БЭМ-обёртка; удалены дубли подключений CSS/JS (2025-10-21)
· v1.0 – добавлен служебный заголовок (2025-10-21)
*/
if (!defined('B_PROLOG_INCLUDED') || B_PROLOG_INCLUDED !== true) {
    die();
}

// Устанавливаем корневой БЭМ-класс для изоляции стилей
$__checkoutRoot = 'checkout';
// SI: Обёртка не должна менять исходную разметку; только контейнер.
echo '<div class="'.$__checkoutRoot.'"><div class="'.$__checkoutRoot.'__inner">';
?>

<? // дальше — штатный код шаблона без изменений ?>
...
$APPLICATION->SetAdditionalCSS('/local/templates/sotbit_origami/assets/css/style-icons.min.css', true);
...
else
{
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="<?if (isset($arParams['IS_IFRAME'])):?> col-md-9 col-12 <?endif;?> row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			<!--	MAIN BLOCK	-->
			<div class="col-md-9 col-12 bx-soa">
				...
				<? else: ?>
					...
					<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							...
							<div class="col-xs-12 col-sm-3 text-md-right"><a href="" class="bx-soa-editstep"><?=$арParams['MESS_EDIT']?></a></div>
						</div>
						<div class="bx-soa-section-content container-fluid"></div>
					</div>
				<? endif ?>
				...
				<div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active">
					<div class="bx-soa-section-title-container">
						<h2 class="bx-soa-section-title col-sm-9">
							<span class="bx-soa-section-title-count"></span><?=$арParams['MESS_BUYER_BLOCK_NAME']?>
						</h2>
						<div class="col-xs-12 col-sm-3 text-md-right"><a href="" class="bx-soa-editstep"><?=$арParams['MESS_EDIT']?></a></div>
					</div>
					<div class="bx-soa-section-content container-fluid"></div>
				...
				<? if ($arParams['BASKET_POSITION'] === 'after'): ?>
					...
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
								<span class="bx-soa-section-title-count"></span><?=$арParams['MESS_BASKET_BLOCK_NAME']?>
							</h2>
							<div class="col-xs-12 col-sm-3 text-md-right"><a href="javascript:void(0)" class="bx-soa-editstep"><?=$арParams['MESS_EDIT']?></a></div>
						</div>
						<div class="bx-soa-section-content container-fluid"></div>
					</div>
				<? endif ?>
						...
						if ($arParams['USER_CONSENT'] === 'Y')
						{
							$APPLICATION->IncludeComponent(
								...
								array(
									...
									'AUTO_SAVE' => 'N',
									'SУBМIT_EVENT_NAME' => 'bx-soa-order-save',
									'REPLACE' => array(
            ...
            <div id="bx-soa-total" class="col-md-3 col-12 bx-soa-sidebar">
                <div class="stiky-wrapper">
                    <div class="main_order_block__top_line top_line_all_price">
                        ...
                        <?if (!isset($arParams['IS_IFRAME'])):?>
                            <a class="main_order_block__link"
                               href="<?= $arParams["PATH_TO_BASKET"] ?>"><?= Loc::getMessage("EDIT_DEFAULT") ?></a>
                        <?else:?>
                            <a class="main_order_block__link"
                               href="<?= $arParams["PATH_TO_BASKET"] ?>" target="_parent"><?= Loc::getMessage("EDIT_DEFAULT") ?></a>
                        <?endif;?>
	...
	<div style="display: none">
		<?
		// Подключение стилей селекторов местоположений, т.к. RestartBuffer() отрезает <head>
		$APPLICATION->IncludeComponent(
    ...
    if (Loader::includeModule('sotbit.origami')) {
        $arParams['DELIVERY_LIST'] = unserialize(Config::get('DELIVERY_LIST')) ?: [];
        $messages['DELIVERY_MESSAGE_PRICE'] = Config::get('DELIVERY_MESSAGE_PRICE');
    }
	?>
	<script>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', function () {
            ...
            if (document.querySelector('.masked-phone')) {
                BX.ready(function() {
                    var phoneMaskSoa = new BX.MaskedInput({
                        mask: '<?=\Sotbit\Origami\Helper\Config::get('MASK')?>',
                        input: document.querySelector('.masked-phone'),
                        placeholder: '_'
                    });
                    phoneMaskSoa.setValue('(__)___-__-__');
                });
            }
	...
	<script>
    <?
    // spike: для потомков городов выводится спец-подсказка
    $city = \Bitrix\Sale\Location\TypeTable::getList([
        'filter' => ['=CODE' => 'CITY'],
        'select' => ['ID']
    ])->fetch();

    // Корректная сборка notFoundPrompt без PHP-вставок внутри строкового литерала
    $notFoundPrompt = '<div class="-bx-popup-special-prompt">'
        . Loc::getMessage('SOA_LOCATION_NOT_FOUND')
        . '.<br />'
        . Loc::getMessage('SOA_LOCATION_NOT_FOUND_PROMPT', [
...
<? // Подключение патча ПОСЛЕ всех инлайн-скриптов и инициализации компонента ?>
<script src="<?=$templateFolder?>/force_expand.js"></script>

<?
// Закрываем обёртку БЭМ
echo '</div></div>';
