...
/*
· Назначение: Патч поведения оформления заказа — отключение сворачивания/переключения секций. Секции всегда развернуты; клики по «Редактировать» и заголовкам не изменяют состояние.
·
· История изменений:
· v1.0 – переопределение методов компонента и восстановление раскрытия после обновлений DOM (2025-10-22)
*/

// SI: Не изменять core-файлы (order_ajax.js). Переопределение выполняется здесь.

(function () {
  'use strict';

  // Восстановление раскрытого состояния всех секций
  function expandAll() {
    document.querySelectorAll('.bx-soa-section').forEach(function (sec) {
      sec.style.display = '';
      sec.classList.add('bx-active');
      sec.setAttribute('data-visited', 'true');
      var cnt = sec.querySelector('.bx-soa-section-content');
      if (cnt) cnt.style.display = '';
    });
  }

  // Блокировка кликов по элементам, сворачивающим секции
  function neutralizeEditLinks() {
    // Снятие уже навешанных обработчиков путём клонирования узлов
    document.querySelectorAll('.bx-soa-editstep, .bx-soa-section-title, .bx-soa-section-title-container a').forEach(function (el) {
    ...
    // Блокировка будущих кликов в режиме перехвата
    document.addEventListener('click', function (e) {
      var trigger = e.target && e.target.closest('.bx-soa-editstep, .bx-soa-section-title, .bx-soa-section-title-container a');
      if (trigger) {
        e.preventDefault();
  ...
  // Переопределение методов компонента, отвечающих за смену/сворачивание секций
  function patchComponent(cmp) {
    try {
      // Не допускаем смену активной секции
      cmp.activeSectionId = null;

      // Глушим методы анимаций/переключений, если присутствуют
      ['showByClick', 'switchSection', 'collapse', 'hideSection', 'show', 'fade'].forEach(function (m) {
        if (typeof cmp[m] === 'function') {
          cmp[m] = function () { return false; };
        }
      });

      // Восстановление развёрнутого состояния сразу после патча
      expandAll();
      neutralizeEditLinks();

      // Мониторинг обновлений DOM (компонент перерисовывает содержимое через AJAX)
      var scope = document.getElementById('bx-soa-order');
      if (scope && !scope.__checkoutObserver) {
        var obs = new MutationObserver(function () {
          // После любых изменений снова раскрывать и нейтрализовать триггеры
          expandAll();
          neutralizeEditLinks();
        });
        obs.observe(scope, { childList: true, subtree: true });
        scope.__checkoutObserver = obs;
      }
    } catch (e) {
      // Ошибки патча игнорируются
    }
  }

  // Ожидание и патчирование BX.Sale.OrderAjaxComponent после загрузки
