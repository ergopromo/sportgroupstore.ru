...
if ($usePict == "Y" && !empty($arSectURL)) {
    ...
    if ($obCache->InitCache(36000, serialize([$arSectURL, $usePict ]), "/sotbit.origami/left_menu")) {
        ...
        while ($arSections = $dbSections->GetNext()) {
            ...
            if ($arSections["PICTURE"]) {
                if ($arSections["PICTURE"]) {
                    $arResizePicture = $ItemImg->resize($arSections['PICTURE'],
                        [
                            'width' => $imgW,
                            'height' => $imgH
                        ],
                        $ItemImg->getResizeType()
                    );
