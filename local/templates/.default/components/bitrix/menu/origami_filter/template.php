...
<? if ($arResult)
{
    ...
    <?
    global $USER;
    if(mb_strpos($li["LINK"], 'personal') && $USER->IsAuthorized()): ?>
        <li class="block_main_left_menu__list_item fonts__main_comment">
            <a class="block_main_left_menu__link" title="<?= $li["TEXT"]; ?>" href="?logout=yes&<?=bitrix_sessid_get();?>">
                <?=GetMessage('SOTBIT_PERSONAL_MENU_EXIT');?>
            </a>
        </li>
    <?endif;?>
