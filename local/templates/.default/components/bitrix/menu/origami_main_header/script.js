...
function updateTopBarMain($topBarMain, $buttonMain, $visibleLinksMain, $hiddenLinksMain, responsiveBreaksMain) {
    if($buttonMain !== undefined){
        ...
        if($visibleLinksMain.width() > availableSpace && window.screen.width>=768) { // Logic when visible list is overflowing the nav
            ...
            for (let i = responsiveBreaksMain.length-1; i >= 0; i--){
                if(availableSpace > responsiveBreaksMain[i]) { // Logic when there is space for another item in the nav
...
function mouseOn(event){
        const visibleLinks = event.target.closest('li[data-show-more]');
        if(visibleLinks !== null){
            visibleLinks.childNodes.forEach(function (item){
                if(item.className === 'category_link__active_content'){
                    item.classList.toggle('showMore');
                }
            });
        }
}
...
function mouseOut(event){
       const disableLinks = document.querySelectorAll('.visible-links__item .showMore');
       disableLinks.forEach(function (item){
           item.classList.remove('showMore');
       });
}

$(document).ready(function() {
    let hiddenLinks = document.querySelector('.hidden-links');
    hiddenLinks.addEventListener('mouseover', function (event){
        mouseOn(event);
    });

    hiddenLinks.addEventListener('mouseout', function (event){
        mouseOut(event);
    });
