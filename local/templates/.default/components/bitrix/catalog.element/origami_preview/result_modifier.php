...
$typeCharacteristics = Config::get('PROP_FILTER_MODE_'.$arResult['TEMPLATE']);
...
$Element = new \Sotbit\Origami\Image\Element($arResult['TEMPLATE']);
...
if($arParams['BRAND_USE'] && $arParams['BRAND_PROP_CODE']){
    $Brand = new \Sotbit\Origami\Brand($arResult['TEMPLATE']);
...
$arResult['VIDEO'] = [];
$videoProp = Option::get('PROP_VIDEO_'.$arResult['TEMPLATE'], SITE_ID);
if(!empty($arResult['PROPERTIES'][$videoProp]['VALUE']) && is_array($arResult['PROPERTIES'][$videoProp]['VALUE'])){
...
if(!empty($arResult['OFFERS']) && is_array($arResult['OFFERS']) && !empty($arResult['SKU_PROPS']) && is_array($arResult['SKU_PROPS'])) {
...
if(!empty($arResult['SKU_PROPS'][$colorCode])) {
    $tmp = [
        $colorCode => $arResult['SKU_PROPS'][$colorCode]
    ];
...
if (Config::get('SHOW_NAVIGATION_'.$arResult['TEMPLATE']) == 'Y') {
    $arFilter = [
