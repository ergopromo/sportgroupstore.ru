...
$itemIds = [
    ...
    'BASE_PRICE_ID'             => $mainId.'_base_price_id',
...
if ($haveOffers)
{
    foreach ($arResult['OFFERS'] as $offerId => $offer) {
        if (!empty($offer['ITEM_ALL_PRICES'])) {
            ...
            foreach ($offer['ITEM_PRICES'] as $priceIt) {
                $allPrices[$offerId][] = [
                    'QUANTITY_HASH' => $priceIt['QUANTITY_HASH'],
                    'QUANTITY_TO' => $priceIt['QUANTITY_TO'],
                    'QUANTITY_FROM' => $priceIt['QUANTITY_FROM'],
                    'MEASURE_RATIO_ID' => $priceIt['MEASURE_RATIO_ID'],
                    'MIN_QUANTITY' => $priceIt['MIN_QUANTITY'],
                    'PRICES' => [
                        1 => $priceIt
                    ]
                ];
...
} else
{
    if (!empty($arResult['ITEM_ALL_PRICES'])) {
        ...
        foreach ($arResult['ITEM_PRICES'] as $priceIt) {
            $allPrices[] = [
                'QUANTITY_HASH' => $priceIt['QUANTITY_HASH'],
                'QUANTITY_TO' => $priceIt['QUANTITY_TO'],
                'QUANTITY_FROM' => $priceIt['QUANTITY_FROM'],
        ...
        //$allPrices = $arResult['ITEM_PRICES'];
    }
                            ...
                            <div class="product_preview_info_block-description">
                                ...
                                <div class="product-preview-price-and-cheaper">
                                    ...
                                    if($arParams["FILL_ITEM_ALL_PRICES"])
                                    {
                                        ...
                                        {
                                            ?>
                                            <div class="product_preview__main-price" id="<?=$itemIds['BASE_PRICE_ID']?>">
                                                <span class="product_preview__btn-show-otherPrices" id="btn-toggle-price"></span>
                                                <div class="product_preview__other_prices_row">
                                                    <span class="product_preview__block__new_price_product fonts__middle_title" data-price-id="<?=$actualItem['ITEM_PRICES'][0]['PRICE_TYPE_ID']?>"><?= $actualItem['ITEM_PRICES'][0]['PRINT_PRICE'] ?></span>
                                                    <?if($arParams['SHOW_OLD_PRICE'] === 'Y' && (isset($arResult['CHECK_DISCOUNT']) || $actualItem['ITEM_PRICES'][0]['RATIO_DISCOUNT'])):?>
                                                        <span class="product_preview__block__old_price_product fonts__small_title js-product-old-price" data-oldprice-id="<?=$actualItem['ITEM_PRICES'][0]['PRICE_TYPE_ID']?>">
                                                            <?= $actualItem['ITEM_PRICES'][0]['PRINT_BASE_PRICE'] ?>
                                                        </span>
                                                    <?endif;?>
                                                    <?if($arParams['SHOW_OLD_PRICE'] === 'Y' && (isset($arResult['CHECK_DISCOUNT']) || $actualItem['ITEM_PRICES'][0]['RATIO_DISCOUNT'])):?>
                                                        <div class="product_preview__block_saving" data-discount-id="<?=$actualItem['ITEM_PRICES'][0]['PRICE_TYPE_ID']?>">
                                                            <?= Loc::getMessage('DETAIL_SAVE') ?>
                                                            <span class="product_preview__block_saving_title">
                                                                <?= $actualItem['ITEM_PRICES'][0]['PRINT_DISCOUNT'] ?>
                                                            </span>
                                                        </div>
                                                    <?endif;?>
                                                </div>
                                            ...
                                            <div class="product_preview__other_prices" id="<?= $itemIds['ALL_PRICES'] ?>">
                                            <?
                                            foreach($arResult["ALL_PRICES_NAMES"]as $id => $idName)
                                            {
                                                ...
                                                ?>
                                                <div class="product_preview__other_prices_row" data-id="<?=$id?>" <?if(empty($allPrice)):?>style="display:none"<?endif;?>>
                                                    <div class="product-detail-title-price fonts__middle_comment">
                                                        <?=$arResult['ALL_PRICES_NAMES'][$id]?>:

                                                    </div>
                                                    <span class="product_preview__block__new_price_product fonts__middle_title" data-price-id="<?=$id?>"><?= $allPrice['PRINT_PRICE'] ?></span>
                                                    <?if($arParams['SHOW_OLD_PRICE'] === 'Y' && (isset($arResult['CHECK_DISCOUNT']) || $allPrice['RATIO_DISCOUNT'])):?>
                                                        <span class="product_preview__block__old_price_product fonts__small_title js-product-old-price" data-oldprice-id="<?=$id?>">
                                                        <?= $allPrice['PRINT_BASE_PRICE'] ?>
                                                    </span>
                                                    <?endif;?>
                                                    <?if($arParams['SHOW_OLD_PRICE'] === 'Y' && (isset($arResult['CHECK_DISCOUNT']) || $allPrice['RATIO_DISCOUNT'])):?>
                                                        <div class="product_preview__block_saving" data-discount-id="<?=$id?>">
                                                            <?= Loc::getMessage('DETAIL_SAVE') ?>
                                                            <span class="product_preview__block_saving_title">
                                    ...
                                    <div class="product-preview-info-block-price" >
                                        <div class="product_preview__block__old_new_price">
                                            <div class="product-detail-title-price fonts__middle_comment">

                                            </div>
                                            <span class="product_preview__block__new_price_product fonts__middle_title js-product-price" id="<?= $itemIds['PRICE_ID'] ?>">
                                                <?= $price['PRINT_PRICE'] ?>
                                            </span>
                                            <?
                                            ...
                                            {
                                                ?>
                                                <span class="product_preview__block__old_price_product fonts__small_title js-product-old-price" id="<?= $itemIds['OLD_PRICE_ID'] ?>" style="display: <?= ($showDiscount ? '' : 'none') ?>;">
                                        ...
                                        {
                                            ?>
                                            <div class="product_preview__block_saving" id="<?= $itemIds['DISCOUNT_PRICE_ID'] ?>" <?if(!$price['DISCOUNT']):?>style="display:none"<?endif;?>>
                                                <?= Loc::getMessage('DETAIL_SAVE') ?>
                                                <span class="product_preview__block_saving_title">
                                    ...
                                    <div class="product_preview__props">
                                        <p class="product_preview__props-title"><?=Loc::getMessage('DETAIL_PROPS')?></p>
                                    <?
                                    ///PROPS
                                                ...
                                                <div class="product-preview-info-block-main-property" data-entity="sku-line-block">
                                                    ...
                                                    <div class="block-property-text">
                                                        ...
                                                        if (Config::get('PROP_DISPLAY_MODE_'.$template) != 'dropdown'):?>
                                                            ...
                                                            ?>
                                                        <? else:?>
                                                            <div class="dropdown-props-wrapper-prev">
                                                                <div class="dropdown-btn angle-down-prev" onclick="window.dropdownTogglePrev(this)">
                                                                    <?= $skuProperty['NAME'] ?>
                                                                </div>
                                                                <div class="dropdown-props-prev">
                                                                    <?
                                                                    foreach($skuProperty['VALUES'] as &$value)
                                                                    {
                                                                        if (!$value['ID'])
                                                            ...
                                                            <style>
                                                                ...
                                                                .dropdown-btn.angle-down-prev::after {
                                                                    content: '';
                                                                    width: 10px;
                                                                    height: 10px;
                                                                ...
                                                                .dropdown-btn.angle-up-prev::after {
                                                                    content: '';
                                                                    width: 10px;
                                                                    height: 10px;
                                                                    border-left: 2px solid #ededed;
                            ...
                            <div class="product_preview_info_block">
                                ...
                                <?
                                    ...
                                    <?if($actualItem['ITEM_PRICES']):?>
                                        <? if (Config::get('SHOW_BUY_OC_TEMPLATE_') == 'Y' && \Bitrix\Main\Loader::includeModule('sotbit.orderphone')): //--temp for display ?>
                                ...
                                <? if (Config::get('SHOW_WANT_GIFT_'.$template) != 'N'): ?>
                                    ...
                                    ?>
                                    <div class="product_preview_info_block_gift" onclick="wantGift('<?= SITE_DIR ?>', '<?= SITE_ID ?>', '<?= $currentUrl ?>', '<?= $arResult['DETAIL_PICTURE']['SRC'] ?>', this, <?=CUtil::PhpToJSObject($price)?>)">
                                        <span>
...
if ($haveOffers)
{
    ...
    $jsParams = [
        'CONFIG'          => [
            ...
            'DISPLAY_ALL_PRICES'       => $arParams["FILL_ITEM_ALL_PRICES"],
        ...
        'MESS'            => [
            ...
            'MANY' => $arParams['~MESS_RELATIVE_QUANTITY_MANY'],
            'FEW' => $arParams['~MESS_RELATIVE_QUANTITY_FEW'],
        ],
        'PRODUCT'         => [
            'ID'         => $arResult['ID'],
