...
use Sotbit\Origami\Helper\Offer;

global $analogProducts, $APPLICATION;
...
Asset::getInstance()->addCss(SITE_TEMPLATE_PATH.'/assets/fonts/share/share.css');
...
if (Loader::includeModule('sotbit.schemaorg') && strpos($APPLICATION->GetCurPage(), "bitrix") === false) {
    ...
    if (is_array($data)) {
        foreach ($data as $k => &$dat) {
            if ($dat['@type'] == 'Product') {
                ...
                if (!empty($arResult['PROPERTIES']['rating']['NAME'])) {
                    $dat['aggregateRating']['name'] = $arResult['PROPERTIES']['rating']['NAME'];
                }

                if (!empty($arResult['PROPERTIES']['rating']['VALUE'])) {
                    $dat['aggregateRating']['ratingValue'] = $arResult['PROPERTIES']['rating']['VALUE'];
                }

                if(!empty($arResult['PROPERTIES']['vote_count']['VALUE'])) {
                    $dat['aggregateRating']['ratingCount'] = $arResult['PROPERTIES']['vote_count']['VALUE'];
                }
...
if(
    Option::get('SHOW_ANALOG_' . $template, SITE_ID) == 'Y' &&
    $arResult['PROPERTIES'][Option::get('ANALOG_PROP_'.$template, SITE_ID)]['VALUE']
){
    $analogProducts = $arResult['PROPERTIES'][Option::get('ANALOG_PROP_'.$template, SITE_ID)]['VALUE'];
}
...
if (
        Config::get('SHOW_PRICE_' . $template) == 'Y'
        && in_array(Config::get('SKU_TYPE_' . $template),
            [
    ...
    if ($arParams['STORES'] && $arResult['OFFERS']) {
        ...
        $rs = CCatalogStoreProduct::GetList([],
            [
                'PRODUCT_ID' => array_keys($idOffers),
        ...
        while ($qnt = $rs->Fetch()) {
            ...
            if ($arParams['SHOW_MAX_QUANTITY'] == 'M') {
                ...
                if (
                        $qnt['AMOUNT'] > 0
                        && $qnt['AMOUNT'] / reset($offer['ITEM_MEASURE_RATIOS'])['RATIO'] < $arParams['RELATIVE_QUANTITY_FACTOR']
                ) {
                    $qnt['AMOUNT'] = '<div class="product_card__block__presence_product_value_sufficient">'
                        ...
                        . ': <span>
                            ...
                            </svg> ' . Loc::getMessage('CT_BCE_CATALOG_RELATIVE_QUANTITY_FEW')
                        . '</span>
					</div>';
                } else {
                ...
                $qnt['AMOUNT'] = '<div class="product_card__block__presence_product_value_many">'
                    . Loc::getMessage('DETAIL_MODIFICATION_AMOUNT') . ':
                        <span>
                            <svg class="product-card_icon-check" width="11px" height="12px">
                                <use xlink:href="/local/templates/sotbit_origami/assets/img/sprite.svg#icon_check_checkbox"></use>
                            </svg> ' . $qnt['AMOUNT'] . ' ' . $offer['ITEM_MEASURE']['TITLE']
                    . '</span>
                </div>';
            }
                                            ...
                                            <div class="product-presence__name-block">
                                                ...
                                                if ($arParams['SHOW_MAX_QUANTITY'] == 'M') {
                                                    if ($offer['CATALOG_QUANTITY'] == 0) {
                                                        $qnt = '<span class="product_card__block__presence_product_value_no"><i class="icon-no-waiting"></i> ' . Loc::getMessage('CT_BCE_CATALOG_RELATIVE_QUANTITY_NO') . '</span>';
                                                    } elseif ($offer['CATALOG_QUANTITY'] / reset($offer['ITEM_MEASURE_RATIOS'])['RATIO'] < $arParams['RELATIVE_QUANTITY_FACTOR']) {
                                                        $qnt = '<span class="product_card__block__presence_product_value_sufficient"><svg class="product-card_icon-check" width="11px" height="12px">
                                                                   <use xlink:href="/local/templates/sotbit_origami/assets/img/sprite.svg#icon_check_checkbox"></use>
                                                                   </svg> ' . Loc::getMessage('CT_BCE_CATALOG_RELATIVE_QUANTITY_FEW') . '</span>';
                                                    } else {
                                                        $qnt = '<span class="product_card__block__presence_product_value_many"><svg class="product-card_icon-check" width="11px" height="12px"><use xlink:href="/local/templates/sotbit_origami/assets/img/sprite.svg#icon_check_checkbox"></use>
                                                                     </svg> ' . Loc::getMessage('CT_BCE_CATALOG_RELATIVE_QUANTITY_MANY') . '</span>';
                                                    }
                                                } else {
                                                    if ($offer['CATALOG_QUANTITY'] == 0) {
                                                    ...
                                                    <div class="product-presence__property-wrapper">
                                                        <? foreach ($arResult['SKU_PROPS'] as $code =>$skuProperty) : ?>
                                                            <div class="product-presence__property">
                                                                ...
                                                                <div><?= $offer['PROPERTIES'][$code]['DISPLAY_VALUE'] ?: $offer['PROPERTIES'][$code]['VALUE'] ?></div>
                                                            </div>
                                                        <? endforeach; ?>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="product-detail-price-and-cheaper">
                                                    <? if ($arParams["FILL_ITEM_ALL_PRICES"] == "Y") {
                                                        ...
                                                        <div class="product_card__other_prices">
                                                            <? foreach ( $arResult['ALL_PRICES_NAMES'] as $priceId => $priceName) : ?>
                                                                <div class="product_card__other_prices_row"
                                                                     data-price="<?= $priceId ?>"
                                                                    <?= (!isset($offer['ITEM_ALL_PRICES'][$offer["ITEM_PRICE_SELECTED"]]["PRICES"][$priceId]['PRINT_PRICE'])) ? "style='display: none;'" : ""; ?>
                                                                >
                                                                    ...
                                                                    <div class="product_card__block__new_price_product"
                                                                        ...
                                                                        <?= $offer['ITEM_ALL_PRICES'][$offer["ITEM_PRICE_SELECTED"]]["PRICES"][$priceId]['PRINT_PRICE'] ?>
                                                                    </div>
                                                                    <? if ($arParams['SHOW_OLD_PRICE'] === 'Y') : ?>
                                                                        <div class="product_card__block__old_price_product"
                                                                             data-oldPrice-id="<?= $id ?>">
                                                                            <?= $offer['ITEM_ALL_PRICES'][$offer["ITEM_PRICE_SELECTED"]]["PRICES"][$priceId]['PRINT_BASE_PRICE'] ?>
                                                                        </div>
                                                                    <? endif; ?>
                                                                    <? if ($arParams['SHOW_OLD_PRICE'] === 'Y') : ?>
                                                                        <div class="product_card__block_saving">
                                                                            ...
                                                                            <span class="product_card__block_saving_title"
                                                                                  data-saving-id="<?= $priceId ?>">
                                                                            <?= $offer['ITEM_ALL_PRICES'][$offer["ITEM_PRICE_SELECTED"]]["PRICES"][$priceId]['PRINT_DISCOUNT'] ?>
                                                                        </span>
                                                                        </div>
                                                                    <? endif; ?>
                                                                </div>
                                                        ...
                                                        ?>
                                                        <div class="product_card__other_prices">
                                                            <div class="product_card__other_prices_row" data-price="1">
                                                                <div class="product-detail-title-price fonts__middle_comment"></div>
                                                                <div class="product_card__block__new_price_product" data-price-id="1">
                                                                    <?= $offer['ITEM_PRICES'][$offer["ITEM_PRICE_SELECTED"]]['PRINT_PRICE'] ?>
                                                                </div>
                                                                <? if ($arParams['SHOW_OLD_PRICE'] === 'Y') : ?>
                                                                    <div class="product_card__block__old_price_product" data-oldPrice-id="1"
                                                ...
                                                <div class="product-presence__basket">
                                                    <div class="product-detail-info-block-basket">
                                                        <? if (
                                                            ...
                                                            && $offer['ITEM_PRICES'][0]['MIN_QUANTITY'] > 0
                                                            && $offer['CAN_BUY']
                                                        ) :
                                                            $offerForRatio = array_values(array_filter($arResult['OFFERS'], fn($a) => $a['ID'] === $offer['ID']))[0];
                                                            $maxQuantity = $offerForRatio['PRODUCT']['QUANTITY'];
                                                            $ratio = $offerForRatio['ITEM_MEASURE_RATIOS'][$offerForRatio['ITEM_MEASURE_RATIO_SELECTED']]['RATIO'];
                                                            $tmpResultMaxQuant = $maxQuantity % $ratio;
                                                            $maxQuantityByRatio = ($tmpResultMaxQuant) === 0 ? $maxQuantity : ($maxQuantity-$tmpResultMaxQuant);
                                                        ?>
                                                            <div class="product_card__block_buy_quantity product-detail-info-block-basket-item">
                                                                ...
                                                                <input class="product_card__block_buy_quantity__input"
                                                                    ...
                                                                    max = "<?= $maxQuantityByRatio ?>"
                                                                    step="<?= $offer['ITEM_MEASURE_RATIOS'][$offer['ITEM_MEASURE_RATIO_SELECTED']]['RATIO'] ?>"
                                                                    data-ratio="<?= $offer['ITEM_MEASURE_RATIOS'][$offer['ITEM_MEASURE_RATIO_SELECTED']]['RATIO'] ?>"
                                                                    data-qnt="<?= $offer['ID'] ?>"
                                                        ...
                                                        <? else: ?>
                                                            <div class="product-detail-info-block-basket-cant-buy">
                                                                <?
                                                            ...
                                                            <div class="product_detail_info_block_offer-wrapper">
                                                                <div class="product_detail_info_block_offer" id="product_check_stock"
                                                                     onclick="checkStock('<?= SITE_DIR ?>', '<?= SITE_ID ?>', '<?= $name ?>', this)"
                                                                >
                                                                    <span class="product_detail_info_block_offer__title"><?= Loc::getMessage('DETAIL_CHECK_STOCK') ?></span>
                                                                </div>
                                                            </div>
                                                        <?endif; ?>

                                                        <? if ($offer['CAN_BUY'] && Config::get('SHOW_BUY_OC_TEMPLATE_') == 'Y' && Loader::includeModule('sotbit.orderphone')): ?>
                                                            <div
                                                ...
                                                <div class="product-presence__address-item">
                                                    ...
                                                    <div class="product-presence__address-rest">
                                                        <div class="product-presence__address-name-block">
                                                            <i class="fa fa-map-marker product-presence__address-name">
                                                                <?=$store['TITLE']?>
                                                            </i>
                                                            <?=$store['AMOUNT']?>
                                                        </div>
                                                        ...
                                                        <? if($store['UF_EMAIL'] && is_array($store['UF_EMAIL'])) : ?>
                                                            <div class="product-presence__address-email">
                                                                <? foreach($store['UF_EMAIL'] as $email) : ?>
                                                                    <a class="product-presence__address-email"
                                                                       href="mailto:<?= $email ?>">
                                                                        <?= $email ?>
                                                                    </a>
                                                                <? endforeach;?>
                                                        ...
                                                        <? if($store['UF_PHONE'] && is_array($store['UF_PHONE'])) :?>
                                                            <div class="product-presence__address-phone">
                                                                <? foreach($store['UF_PHONE'] as $phone) :?>
                                                                    <a class="product-presence__address-link"
                                                                       href="tel:<?= \SotbitOrigami::showDigitalPhone($phone) ?>"><?= $phone ?></a>
                                                                <? endforeach; ?>
                                                            </div>
                                                        <? endif; ?>
                                                    </div>
...
if (Config::get('ACTIVE_TAB_AVAILABLE_' . $template) == 'Y') {
    ...
    if (CCatalogSKU::IsExistOffers($arResult['ID'], $arResult['IBLOCK_ID'])) {
        $arItemsSKU = CCatalogSKU::getOffersList(
            [$arResult['ID']],
            $arResult['IBLOCK_ID'],
            [],
            []
        ...
        foreach ($arItemsSKU[$arResult['ID']] as $item) {
            $dbItem = CCatalogStoreProduct::GetList(
                [],
                [
                    'PRODUCT_ID' => $item,
                    'STORE_ID' => $arParams['STORES']
            ...
            <div class="availability-item" data-id="<?=$qnt['PRODUCT_ID']?>">
                ...
                <div class="availability-item__content-tile-wrapper availability-item__content-tile-wrapper--tablet">
                    <? if($store['ADDRESS']) : ?>
                        ...
                        <?
                            ...
                            $lon = $qnt['STORE_GPS_S'];
                        ?>
                        <a href="#map-test" onclick="showOnTheMap('<?=$lon?>','<?=$lat?>', '<?=$arParams['STORES_MAP']?>');" class="availability-item__content-link-map main-color-txt">
                ...
                <div class="availability-item__content">
                    ...
                    <div class="availability-item__content-info">
                        <? if ($arParams['SHOW_MAX_QUANTITY'] == 'M') {
                            $measure = reset($arResult['ITEM_MEASURE_RATIOS'])['RATIO'] == 0 ? INF : ($qnt['AMOUNT'] / reset($arResult['ITEM_MEASURE_RATIOS'])['RATIO']);
                            if ($qnt['AMOUNT'] == 0) {
                                $qnt['AMOUNT'] = '
                            ...
                            } elseif ($measure < $arParams['RELATIVE_QUANTITY_FACTOR']) {
                                $qnt['AMOUNT'] = '
                            <div class="availability-item__amount availability-item__amount--few">
                                ...
                                </svg>' . Loc::getMessage('CT_BCE_CATALOG_RELATIVE_QUANTITY_FEW') . '</p>
                            </div>';
                            } else {
                                $qnt['AMOUNT'] = '
                        <div class="availability-item__amount availability-item__amount--lot">
                            ...
                            <p class="availability-item__amount-content">
                               <svg class="availability-item__amount-content-icon" width="10" height="10">
                                ...
                                </svg>' . Loc::getMessage('CT_BCE_CATALOG_RELATIVE_QUANTITY_MANY') . '
                               </p>
                        </div>';
                            }
                        } elseif ($qnt['AMOUNT'] > 0) {
                        ...
                        <div class="availability-item__metro">
                            ...
                            <div class="availability-item__metro">
                                <p class="detailed-tabs__availability-metro-name"><?= $store['UF_METRO'] ?: '-'; ?></p>
                            </div>
                            ...
                            if ($store['UF_EMAIL']) {
                                ...
                                <div class="availability-item__email">
                                    <? foreach ($store['UF_EMAIL'] as $email) {
                                        ?>
                                        <a href="mailto:<?= $email ?>"
                                           class="availability-item__email-name main-color-txt"><?= $email ?></a>
                                        <?
                                    } ?>
                                </div>
                                <?
...
$useRegion = (bool)Config::get('USE_REGIONS') == 'Y';
...
?>
	...
	<script>
        ...
        <? if($arParams["AJAX_MODE"] === "Y"){ ?>
            var showOnTheMap = (lon, lat, mapType) => {
                if (mapType === 'YANDEX') {
        ...
        <? if($_REQUEST['bxajaxid']){ ?>
            <?= $arResult['OBNAME'] ?>.init();
        <? } ?>
	</script>

<?
if(Bitrix\Main\Loader::includeModule('sotbit.opengraph')) {
    if(!empty($templateData["ITEM"]["JS_OFFERS"][0]["DETAIL_PICTURE"]["SRC"])) {
