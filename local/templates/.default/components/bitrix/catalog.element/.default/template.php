...
Asset::getInstance()->addCss(SITE_TEMPLATE_PATH . "/assets/fonts/font-awesome/css/font-awesome.css");
...
$template = '';
if ($this->__name == 'origami_no_tabs') {
    $template = 'NO_TABS';
}

if (
    Config::get('OFFER_LANDING') == 'Y' &&
    !empty($arResult['JS_OFFERS']) &&
    !empty($arResult['OFFERS']) &&
    \SotbitOrigami::isCheckOfferPage()
) {
...
$templateData['JS_OBJ'] = $arResult['OBNAME'];
$name = $arResult['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE'] ?: $arResult['NAME'];
$title = $arResult['IPROPERTY_VALUES']['ELEMENT_DETAIL_PICTURE_FILE_TITLE'] ?: $arResult['NAME'];
...
if ($haveOffers) {
    ...
    foreach ($arResult['OFFERS'] as $offer) {
        ...
        if ($offer['MORE_PHOTO_COUNT'] >= 1 && !$showSliderControls) {
...
$coord_lat = $arCoordsArray[0][0];
$coord_lon = $arCoordsArray[0][1];
if ($arParams['STORES_MAP'] == 'YANDEX') {
    $position['yandex_lat'] = $coord_lat;
    $position['yandex_lon'] = $coord_lon;
    $position['yandex_scale'] = 14;
} else {
    $position['google_lat'] = $coord_lat;
    $position['google_lon'] = $coord_lon;
    $position['google_scale'] = 14;
}

foreach ($arCoordsArray as $arCoord) {
    $position['PLACEMARKS'][] = [
        'LON' => $arCoord[1],
        'LAT' => $arCoord[0]
    ];
...
$blockID = Random::getString(8);
$curRequest = Application::getInstance()->getContext()->getRequest();
?>
<div id="<?= $itemIds['ID'] ?>" itemscope itemtype="http://schema.org/Product"
                ...
                <div id="<?= $itemIds['BIG_SLIDER_ID'] ?>"
                    ...
                    if ($showSliderControls)
                    {
                        if ($haveOffers)
                        {
                            ...
                            {
                                if ((!isset($offer['MORE_PHOTO_COUNT']) || $offer['MORE_PHOTO_COUNT'] <= 0) && !$arResult['VIDEO'] )
                                {
                                    continue;
                                }
                                $strVisible = $arResult['OFFERS_SELECTED'] == $keyOffer ? '' : 'none';
                                $strActive = $arResult['OFFERS_SELECTED'] == $keyOffer ? 'active' : '';
                                ?>
                                <div class="product-item-detail-slider-controls-block swiper-container gallery-thumbs <?=$strActive?>" id="<?= $itemIds['SLIDER_CONT_OF_ID'].$offer['ID'] ?>" style="display: <?= $strVisible ?>;">
                                    <div class="swiper-wrapper">
                                        <?
                                        if($offer['MORE_PHOTO']) {
                                            foreach ($offer['MORE_PHOTO'] as $keyPhoto => $photo) {
                                                $photoSmall = $photo['SMALL'] ?: $photo['ORIGINAL'];
                                                ?>
                                                <div
                                                    class="product-item-detail-slider-controls-image swiper-slide<?= ($keyPhoto
                                                    == 0 ? ' active' : '') ?>"
                                                    data-entity="slider-control"
                                                    data-value="<?= $offer['ID']
                                                    . '_'
                                                    . $photo['ORIGINAL']['ID'] ?>">
                                                    <img src="<?= $photoSmall['SRC'] ?>" alt="<?= $alt ?>"
                                                         title="<?= $title ?>">
                                                </div>
                                                <?
                                            }
                                        ...
                                        if ($arResult['VIDEO'])
                                        {
                                            foreach ($arResult['VIDEO'] as $i => $video)
                                            {
                                                ...
                                                <div class="product-item-detail-slider-controls-video swiper-slide" data-entity="video" data-value="<?= $i ?>">
                                                    <div class="video-slider-control"><?= $video?></div>
                ...
                <div class="col-sm-12">
                    <div class="product_detail_title_wrapper">
                        <? if ($arParams['DISPLAY_NAME'] === 'Y'): ?>
                            <h1 class="product_detail_title fonts__middle_title js-product-name" data-entity="product-name"><?=$name?></h1>
                        <?endif;?>
                        <? if ($arResult['BRAND']): ?>
            ...
            <div class="row product_detail_info">
                    ...
                    <div class="product_detail_info_block">
                                            ...
                                            <span class="product_card__block__presence_product_value_many" data-entity="quantity-limit-value">
                                                ...
                                                if($arParams['SHOW_MAX_QUANTITY'] === 'M')
                                                {
                                                    if($actualItem['CATALOG_QUANTITY'] == 0)
                                                    {
                                                        ...
                                                        <?
                                                    } elseif($measureRatio && ((float)$actualItem['CATALOG_QUANTITY'] / $measureRatio >= $arParams['RELATIVE_QUANTITY_FACTOR'])) {
                                                        ?>
                                                        <span class="product_card__block__presence_product_value_many" data-entity="quantity-limit-value">
                        ...
                        <?if(!$haveOffers || in_array(Config::get('SKU_TYPE_' . $template), ['ENUMERATION', 'COMBINED'])):?>
                            <div class="product-detail-price-and-cheaper">
                                <?
                                if($arParams["FILL_ITEM_ALL_PRICES"] == "Y" && !empty($arResult["ALL_PRICES_NAMES"]))
                                {
                                    ?>
                                    <div class="product_card__other_prices" id="<?= $itemIds['ALL_PRICES'] ?>">
                                        ...
                                        foreach ($arResult["ALL_PRICES_NAMES"]as $id => $idName)
                                        {
                                            ...
                                            <div class="product_card__other_prices_row" data-id="<?=$id?>" <?if(empty($allPrice)):?>style="display:none"<?endif;?>>
                                                ...
                                                <?endif;?>
                                                <?if($arParams['SHOW_OLD_PRICE'] === 'Y' && (isset($arResult['CHECK_DISCOUNT']) || $allPrice['RATIO_DISCOUNT'])):?>
                                                    <div class="product_card__block_saving" <?if(!$allPrice['RATIO_DISCOUNT']):?>style="display:none"<?endif;?>>
                                                        <?= Loc::getMessage('DETAIL_SAVE') ?>
                                                        <span class="product_card__block_saving_title" data-discount-id="<?=$id?>">
                                ...
                                <?if($arParams["FILL_ITEM_ALL_PRICES"] != "Y"):?>
                                    <div class="product-detail-info-block-price">
                                        ...
                                        <span class="product_card__block__new_price_product fonts__middle_title js-product-price" id="<?= $itemIds['PRICE_ID'] ?>" data-price-id="1"><?= $price['PRINT_PRICE'] ?></span>
                                            <?
                                        ...
                                        {
                                            ?>
                                            <div class="product_card__block_saving" <?if(!$price['DISCOUNT']):?>style="display:none"<?endif;?>>
                                                <?= Loc::getMessage('DETAIL_SAVE') ?>
                                                <span class="product_card__block_saving_title" id="<?= $itemIds['DISCOUNT_PRICE_ID'] ?>">
                        ...
                        <? if (!in_array(Config::get('SKU_TYPE_' . $template), ['ENUMERATION', 'COMBINED']) && $haveOffers): ?>
                            ...
                            <? if (Config::get('SHOW_WANT_GIFT_' . $template)!= 'N' && (!$haveOffers || !in_array(Config::get('SKU_TYPE_' . $template), ['LIST_OF_MODIFICATIONS']))): ?>
                                <?
                                $currentUrl = $curRequest->isHttps() ? "https://" : "http://";
                                $currentUrl .= $_SERVER["HTTP_HOST"];
                                $currentUrl .= $arResult['DETAIL_PAGE_URL'];
                                ?>
                                <div class="product_detail_info_block_gift" onclick="wantGift('<?= SITE_DIR ?>', '<?= SITE_ID ?>', '<?= $currentUrl ?>', '<?= $arResult['DETAIL_PICTURE']['SRC'] ?>', this, <?=CUtil::PhpToJSObject($price)?>)">
			                            <span>
                ...
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                    <div id="right_detail_card"
                        ...
                        <?
                        $addProps = unserialize(\Sotbit\Origami\Config\Option::get('DETAIL_ADD_PROPS_' . $template, SITE_ID));
                        ...
                        <?if($arResult["SHOW_BUY"]):?>
                            <div class="product_detail_info_block product_detail_info_block-buy">
                                <div class="product-detail-info-block-basket">
                                    <?if(
                                        Config::get('SHOW_CHECK_STOCK_'.$template) == 'Y' &&
                                        ($haveOffers || (!$haveOffers && !$arResult['CAN_BUY']))
                                    ):?>
                                    ...
                                    <div class="product-detail-info-block__buy"><?
                                        if ($arParams['USE_PRODUCT_QUANTITY'] && $price['MIN_QUANTITY'] > 0 && $actualItem['CAN_BUY'])
                                        {
                                            ?>
                                        ...
                                        <? if (Config::get('SHOW_BUY_OC_TEMPLATE_') == 'Y' && Loader::includeModule('sotbit.orderphone') && $actualItem['CATALOG_QUANTITY'] > 0): ?>
                                ...
                                <? if (Config::get('SHOW_WANT_GIFT_' . $template) != 'N'): ?>
                                    ...
                                    <div class="product_detail_info_block_gift product_detail_info_block_gift--gift"
                                         onclick="wantGift('<?= SITE_DIR ?>', '<?= SITE_ID ?>', '<?= $currentUrl ?>', '<?= $arResult['DETAIL_PICTURE']['SRC'] ?>',  this, <?=CUtil::PhpToJSObject($price)?>)">
                                        <span>
                                             <svg class="product_detail_info_block_gift-icon" width="13" height="13">
                                                    <use xlink:href="/local/templates/sotbit_origami/assets/img/sprite.svg#icon_gift"></use>
                                            </svg>
                        ...
                        <div class="product_detail__nav-content product_detail__nav-content--prev">
                            <?if($arResult['PREV_ITEM']['PREVIEW_PICTURE'] || $arResult['PREV_ITEM']['DETAIL_PICTURE']):?>
                                <div class="product_detail__nav-img">
                                    <img src="<?=CFile::GetPath($arResult['PREV_ITEM']['PREVIEW_PICTURE'] ?: $arResult['PREV_ITEM']['DETAIL_PICTURE'])?>" alt="">
                                </div>
                            <?endif;?>
                    ...
                    <a href="<?=$arResult['NEXT_ITEM']['DETAIL_PAGE_URL']?>" class="product_detail__nav-next">
                        ...
                        <div class="product_detail__nav-content product_detail__nav-content--next">
                            <?if($arResult['NEXT_ITEM']['PREVIEW_PICTURE'] || $arResult['NEXT_ITEM']['DETAIL_PICTURE']):?>
                                <div class="product_detail__nav-img">
                                    <img src="<?=CFile::GetPath($arResult['NEXT_ITEM']['PREVIEW_PICTURE'] ?: $arResult['NEXT_ITEM']['DETAIL_PICTURE'])?>" alt="">
                                </div>
                            <?endif;?>
                            <div class="product_detail__nav-description">
    ...
    <?if(Loader::includeModule('sotbit.reviews') && in_array('COMPLEX_REVIEWS', array_column($arResult['TABS'], 'TYPE'))):?>
                ...
                <div class="review-auth">
                    <div class="review-auth__mobile-toggles">
                        <div class="review-auth__mobile-toggles-btn"
                             onclick="toggleAuth(this)"><?= Loc::getMessage('REVIEWS_AUTH'); ?></div>
                        <div class="review-auth__mobile-toggles-btn"
                             onclick="toggleAuth(this, true)"><?= Loc::getMessage('REVIEWS__REGISTER'); ?></div>
                    </div>
                    <div class="review-auth__container">
                        <div class="form-auth">
                            <div class="review-auth__inner-container">
                                <h2 class="review-auth__title"><?= Loc::getMessage('REVIEWS_AUTH'); ?></h2>
                        ...
                        <div class="form-reg">
                            <div class="review-auth__inner-container">
                                <h2 class="review-auth__title"><?= Loc::getMessage('REVIEWS__REGISTER'); ?></h2>
    ...
    <?if ($arResult['TABS'])
    {
        ...
        <div class="detailed-tabs">
            <div class="swiper-container">
                <ul class="detailed-tabs__tabs swiper-wrapper tabs-caption" data-id-element="<?=$arResult['ID']?>">
                    <?foreach ($arResult['TABS'] as $i => $tab):?>
                        <li class="review-menu-tabs__item detailed-tabs__tabs-item swiper-slide <?= ($i == 0) ? 'active' : '' ?>"
                            id="TAB_<?= $tab['TYPE'] ?>"
                            data-ajax="Y">
                            <p><?= $tab['NAME'] ?></p>
                        </li>
                ...
                foreach ($arResult['TABS'] as $i => $tab)
                {
                    switch ($tab['TYPE'])
                    {
                        ...
                        case 'COMPLEX_REVIEWS':
                            ?>
                            <div class="detailed-tabs__content detailed-tabs__complex_reviews <?= ($i == 0) ? 'active' : '' ?>">
                            ...
                            <?
                            break;
                        case 'COMPLEX_COMMENTS':
                            ?>
                            <div class="detailed-tabs__content detailed-tabs__complex_comments <?= ($i == 0) ? 'active' : '' ?>">
                        ...
                        case 'COMPLEX_QUESTIONS':
                            ?>
                            <div class="detailed-tabs__content detailed-tabs__complex_questions <?= ($i == 0) ? 'active' : '' ?>">
...
if ($arResult['OFFER_GROUP']) {
    ...
    } elseif ($arResult['MODULES']['catalog']) {
        $APPLICATION->IncludeComponent(
            'bitrix:catalog.set.constructor',
            'origami_default',
            [
...
if ($arResult['CATALOG'] && $arParams['USE_GIFTS_DETAIL'] == 'Y' && ModuleManager::isModuleInstalled('sale'))
{
    ...
    <div data-entity="parent-container">
        ...
        $APPLICATION->IncludeComponent(
            ...
            + [
                'OFFER_ID' => $arResult['OFFERS'][$arResult['OFFERS_SELECTED']]['ID'] ?: $arResult['ID'],
...
if ($haveOffers)
{
    foreach ($arResult['OFFERS'] as $offerId => $offer) {
        if (!empty($offer['ITEM_ALL_PRICES'])) {
            $allPrices[] = $offer['ITEM_ALL_PRICES'];
        } else {
            if(!empty($offer['ITEM_PRICES'])){
                foreach ($offer['ITEM_PRICES'] as $price) {
                    $allPrices[$offerId][] = [
                        'QUANTITY_HASH' => $price['QUANTITY_HASH'],
                        'QUANTITY_TO' => $price['QUANTITY_TO'],
                        'QUANTITY_FROM' => $price['QUANTITY_FROM'],
                        'MEASURE_RATIO_ID' => $price['MEASURE_RATIO_ID'],
    ...
    if (!empty($arResult['ITEM_ALL_PRICES'])) {
        $allPrices = $arResult['ITEM_ALL_PRICES'];
    } else {
...
if ($haveOffers)
{
    ...
    foreach ($arResult['JS_OFFERS'] as $ind => &$jsOffer)
    {
        ...
        if ($arResult['SHOW_OFFERS_PROPS'] && !empty($jsOffer['DISPLAY_PROPERTIES']))
        {
            foreach ($jsOffer['DISPLAY_PROPERTIES'] as $property)
            {
                $current
                    = '<li class="detailed-tabs-list__item" data-type="forOffers">
                            <p class="detailed-tabs-list__item-name">' . $property['NAME'] . ': </p>
                            <p class="detailed-tabs-list__item-value">' . (
    ...
    $jsParams = [
        'CONFIG' => [
            ...
            'USE_ENHANCED_ECOMMERCE' => $arParams['USE_ENHANCED_ECOMMERCE'],
            'DATA_LAYER_NAME' => $arParams['DATA_LAYER_NAME'],
            'BRAND_PROPERTY' => $arResult['DISPLAY_PROPERTIES'][$arParams['BRAND_PROPERTY']]['DISPLAY_VALUE'] ?: null,
        ],
        'PRODUCT_TYPE' => $arResult['CATALOG_TYPE'],
        'VISUAL' => $itemIds,
        'ACTIVE_AJAX' => $_REQUEST['bxajaxid'] ?: 'N',
        'DEFAULT_PICTURE' => [
    ...
    $jsParams = [
        ...
        'PRODUCT' => [
            ...
            'SLIDER_COUNT' => intval($arResult['MORE_PHOTO_COUNT']) + count($arResult['VIDEO']),
...
<script>
    BX.message({
        ...
        RELATIVE_QUANTITY_MANY_DETAIL: '<?=$arParams['~MESS_RELATIVE_QUANTITY_MANY']?>',
        RELATIVE_QUANTITY_FEW_DETAIL: '<?=$arParams['~MESS_RELATIVE_QUANTITY_FEW']?>',
        RELATIVE_QUANTITY_NO_DETAIL: '<?=$arParams['~MESS_RELATIVE_QUANTITY_NO']?>',
        SITE_ID: '<?=CUtil::JSEscape($component->getSiteId())?>',
        WISH_TO: '<?=GetMessageJS('WISH_TO')?>',
        WISH_IN: '<?=GetMessageJS('WISH_IN')?>',
    });
    var <?=$arResult['OBNAME']?> = new JCCatalogElement(<?=CUtil::PhpToJSObject($jsParams, false, true)?>);
    <? if($arParams["AJAX_MODE"] === "Y"){ ?>
        var tabs = new TabsDetailed({
