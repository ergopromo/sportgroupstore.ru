...
function setDisplayPropsDescription(&$arJSOffers, $arOffers) {
    foreach ($arJSOffers as &$jsOffer) {
        if($arOffers[$jsOffer['CODE']] !== null && $arOffers[$jsOffer['CODE']]['DESCRIPTION'] !== null) {
            $jsOffer['DESCRIPTION'] = $arOffers[$jsOffer['CODE']]['DESCRIPTION'];
        }
    }
}
...
$propGroupType = Config::get('PROPERTY_GROUPER_' . $template);
...
if($arResult['JS_OFFERS']){
    foreach($arResult['JS_OFFERS'] as $i => $offer)
    {
        ...
        if ($propGroupType == 'NO') {
            setDisplayPropsDescription($arResult['JS_OFFERS'][$i]['DISPLAY_PROPERTIES'],
                $arResult['OFFERS'][$i]['DISPLAY_PROPERTIES']);
        }
    }
}
...
$arResult['BRAND'] = [];
if(!empty($arParams['BRAND_PROP_CODE']))
{
    $Brand = new \Sotbit\Origami\Brand($template, SITE_ID);
...
$videoProp = Option::get('PROP_VIDEO_'.$template, SITE_ID);
...
$tabs = unserialize(Config::get('TABS_'.$template, SITE_ID));
if($tabs){
    foreach($tabs as $tab){
        ...
        if($tab == 'PROPERTIES' &&
            (empty($arResult['DISPLAY_PROPERTIES']) || count($arResult['DISPLAY_PROPERTIES']) < 1 )
        ) {
            continue;
        }
        ...
        if(Config::get('ACTIVE_TAB_'.$tab.'_'.$template, SITE_ID) == 'Y'){
            $name = Option::get('NAME_TAB_'.$tab.'_'.$template, SITE_ID);

            if($tab == 'DOCS'){
                $propDoc = Option::get('PROP_TAB_DOCS_'.$template, SITE_ID);
            ...
            if($tab == 'VIDEO'){
                $propVideo = Option::get('PROP_TAB_VIDEO_'.$template, SITE_ID);
    ...
    foreach ($arElementNav as $key => $item) {
        if ($item['ID'] == $arResult['ID']) {
            if ($arElementNav[$key+1]) {
                ...
                $nextItem = CIBlockElement::GetList(
                    [$arSort],
                    $arFilter,
                    false,
                    ["nPageSize"=>1],
...
$arResult['OBNAME'] = 'ob' . preg_replace('/[^a-zA-Z0-9_]/', 'x', $mainId);
