...
<div class="sotbit_questions_personal_questions">
    ...
    <table class="table-personal-questions" data-site-dir="<?= SITE_DIR ?>" data-sort="" data-by="">
        ...
        <tr>
            ...
            <th data-sort="bill"><?= Loc::getMessage(CSotbitReviews::iModuleID . "_QUESTIONS_PERSONAL_BILLS") ?>
                (<?= $arResult['CURRENCY'] ?>)
            </th>
        </tr>
        </thead>
        ...
        if (isset($arResult['QUESTIONS']) && is_array($arResult['QUESTIONS']) && $arResult["QUESTIONS_CNT"] > 0) {
            foreach ($arResult['QUESTIONS'] as $k => $Question) {
                ...
                <tr>
                    ...
                    <td class="question" data-name="question"
                        data-value="<?= $Question['ELEMENT_NAME'] ?> <?= $Question['QUESTION'] ?>"><a
                            href="<?= $Question['ELEMENT_URL'] ?>"><?= $Question['ELEMENT_NAME'] ?></a>
                        <div class="text">
                            <?= $Question['QUESTION'] ?>
                        </div>
                    ...
                    <td class="status status-<?= $Question['STATUS'] ?>" data-name="status"
                        data-value="<?= $Question['STATUS'] ?>">
                    ...
                    <td class="options" data-name="options" data-value="<?= $k ?>">
                        <div class="wrap-options">
                            <i class="fa fa-pencil-square-o personal-edit" aria-hidden="true"
                               data-id="<?= $Question['ID'] ?>"></i>
                            <i class="fa fa-times personal-delete" aria-hidden="true"
                               data-id="<?= $Question['ID'] ?>"></i>
                        </div>
                    </td>
    ...
    <div class="questions-popup" id="modal-questions-popup">
        ...
        <div class="question-add-block">
            ...
            <form class="question" id="add_question" action="javascript:void(null);" enctype="multipart/form-data">
                ...
                <? if (isset($arResult['RECAPTCHA2_SITE_KEY']) && !empty($arResult['RECAPTCHA2_SITE_KEY'])): ?>
                    <div data-captcha-question="Y" id="recaptcha-question-<?= $arResult["QUESTIONS_CNT"] ?>"
                         class="captcha-block"></div>
                <? endif; ?>
                <input type="submit" name="submit" id="question_submit"
                       value="<?= GetMessage(CSotbitReviews::iModuleID . "_QUESTIONS_SUBMIT_VALUE") ?>"/>
            </form>
        </div>
...
<div id="PersonalQuestionsParams"
